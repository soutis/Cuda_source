...
__constant__  double Filter[FILTER_LENGTH];   // gia na apothikeusw tous suntelestes tou filtrou
...
__global__ void tiled_row_Kernel(double *Dst, double *Src,int imageW, int imageH, int filterR) {
    ...
    __shared__ double data[TILE_HEIGHT * (FILTER_LENGTH - 1 +TILE_WIDTH)];
...
__global__ void tiled_column_Kernel(double *Dst, double *Src,int imageW, int imageH, int filterR) {
     ...
     // allocate 1D tile in __shared__ memory
    __shared__ double data[TILE_WIDTH * (TILE_HEIGHT + FILTER_LENGTH - 1)];
...
int main(int argc, char **argv) {
    ...
    *d_Buffer;       //Buffer sto device gia endiameso apotelesma apo thn row sth column ston kernel
    ...
    double average_1=0;  //mesos xronos gia thn gpu xwris to tile
    double average_2=0;  //mesos xronso gia thn gpu me to tile
    ...
    h_Input     = (double *)malloc(newW * newH * sizeof(double));
    h_Buffer    = (double *)malloc(newW * newH * sizeof(double));
    h_OutputCPU = (double *)malloc(newW * newH * sizeof(double));
    h_OutputGPU1 = (double *)malloc(newW * newH * sizeof(double));
    h_OutputGPU2 = (double *)malloc(newW * newH * sizeof(double));
    ...
    err = cudaSuccess;
    err = cudaMalloc((void**)&d_Input, newW * newH * sizeof(double));
    err = cudaMalloc((void**)&d_OutputGPU1, newW * newH * sizeof(double));
    err = cudaMalloc((void**)&d_OutputGPU2, newW * newH * sizeof(double));
    err = cudaMalloc((void**)&d_Filter, ((2*FILTER_RADIUS)+1)*sizeof(double)); 
    err = cudaMalloc((void**)&d_Buffer, newW * newH * sizeof(double)); 
   ...
   //arxikopoiw me floats wste na exw megaluterh anakriveia sta apotelesmata mou
	...
	for(j=0; j < newH; j++){
		if(i<FILTER_RADIUS || j<FILTER_RADIUS  || i >= (imageW+FILTER_RADIUS) || j>=(imageH+FILTER_RADIUS)){		
			...
			h_Input[j*newW+i] = 0;
		      ...
		      h_Input[j*newH+i] = (double)(rand()/(double)RAND_MAX);			
		}
    ...
    //Load h_Input and h_Filter to device memory
     err = cudaMemcpy(d_Input,h_Input,newW * newH * sizeof(double),cudaMemcpyHostToDevice);
     err = cudaMemcpy(d_Filter,h_Filter,((2*FILTER_RADIUS)+1)*sizeof(double),cudaMemcpyHostToDevice);  
     err = cudaMemcpyToSymbol(Filter,h_Filter,((2*FILTER_RADIUS)+1)*sizeof(double),0,cudaMemcpyHostToDevice);
   ...
   /////  cuda events for tiled gpu time calculation  /////
    ...
    err = cudaMemcpy(h_OutputGPU1,d_OutputGPU1,newW * newH * sizeof(double),cudaMemcpyDeviceToHost);
    err = cudaMemcpy(h_OutputGPU2,d_OutputGPU2,newW * newH * sizeof(double),cudaMemcpyDeviceToHost);
