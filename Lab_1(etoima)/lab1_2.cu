...
__global__ void row_Kernel(float *Dst, float *Src, float *Filter,int imageW, int imageH, int filterR) {
	...
	int d = threadIdx.x + k;
        ...
        if(d >= 0 && d < imageW) {
           sum += Src[threadIdx.y * imageW + d] * Filter[filterR - k];    
        }	     
        Dst[threadIdx.y * imageW + threadIdx.x]=sum;
...
__global__ void column_Kernel(float *Dst, float *Src, float *Filter,int imageW, int imageH, int filterR) {
	...
	int d = threadIdx.y + k;
        ...
        if(d >= 0 && d < imageH) {
           sum += Src[d * imageW + threadIdx.x] * Filter[filterR - k];    
        }	     
        Dst[threadIdx.y * imageW + threadIdx.x]=sum;
      }
